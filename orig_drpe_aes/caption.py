...
def caption(args_, word_map_):
    beam_size = args_.beam_size
    vocab_size = len(word_map_)
    loader = torch.utils.data.DataLoader(CaptionTestDataset(args_.data_folder, args_.file_name, args_.mode),
                                         batch_size=1, shuffle=False, num_workers=4, pin_memory=True)
    image_names_ = list()
    sequences_ = list()
    with torch.no_grad():
        if args_.mode == "drpe":
            for i, (real_image, imag_image, name) in enumerate(tqdm(loader,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', '-m', default="models/BEST_checkpoint_flickr8k_5_min_word.pth.tar",
                        help='path to model')
    ...
    parser.add_argument('--cnn_name', default="vit", help='which model does cnn use?')
    parser.add_argument('--image_size', default=224, type=int, help='image size')
    parser.add_argument('--mode', default="aes")
    ...
    caps_3 = list()
    image_names, sequences = caption(args, word_map)
    ...
    folder = 'results_csv/' + args.data_name + '/' + str(args.image_size) + '/' + 'models_' + args.mode
    if not os.path.exists(folder):
        os.makedirs(folder)
    name_csv = args.checkpoint.split('/')[-1][:-8] + '.csv'
