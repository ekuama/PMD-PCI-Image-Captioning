...
def train(args_, train_loader_, encoder_, decoder_, criterion_, encoder_optimizer_, decoder_optimizer_, epoch_):
    ...
    start = time.time()
    if args_.mode == "drpe":
        for i_t, (real_image, imag_image, caps, caplens) in enumerate(train_loader_):
        ...
        for i_t, (image, caps, caplens) in enumerate(train_loader_):
            data_time.update(time.time() - start)

            pmd_image = image.to(device)
            caps = caps.to(device)
            caplens = caplens.to(device)
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(
        CaptionDataset(args.data_folder, args.file_name, 'TRAIN', args.mode),
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        save_checkpoint(file_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
                        ...
                        args.mode, args.image_size, args.data_name)

    with h5py.File(os.path.join('models/' + args.data_name + '/' + str(args.image_size) + '/' +
                                'models_' + args.mode + '/' + 'metrics_vit.hdf5'), 'a') as h_:
