...
from datasets import CaptionDataset, CaptionFedBaseDataset, FederatedDataset
from models import CNN_Encoder, CNN_Encoder2
from transformer import Transformer
from utils import AverageMeter, clip_gradient, accuracy, get_eval_score, save_fl_checkpoint
...
def train_model(args_, train_dataset_, val_loader_, g_encoder_, g_decoder_, criterion_, file_name_):
    ...
    client_datasets = [FederatedDataset(train_dataset_, client_id, args_.num_clients)
    ...
    for epoch_g in range(args_.epoch_global):
        ...
        for client_id in range(args_.num_clients):
            ...
            for epoch_l in range(args_.epoch_local):
                ...
                start = time.time()
                if args_.mode == "drpe":
                    for i_t, (real_image, imag_image, caps, caplens) in enumerate(client_dataloader):
                        ...
                        feat = local_encoder(real_image, imag_image)
                    ...
                    for i_t, (image, caps, caplens) in enumerate(client_dataloader):
                        data_time.update(time.time() - start)
        ...
        save_fl_checkpoint(file_name_, epoch_g, epochs_since_improvement, g_encoder_, g_decoder_, metrics, is_best_bleu,
                           is_best_cider, args.mode, args.image_size, args.data_name, args.num_clients)
...
def validate(args_, val_loader_, encoder_, decoder_, criterion_):
    ...
    with torch.no_grad():
        ...
        if args_.mode == "drpe":
            for i_t, (real_image, imag_image, caps, caplens, allcaps) in enumerate(val_loader_):
                ...
                feat = encoder_(real_image, imag_image)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attention_method', default="ByPixel", help='which attention method to use?')
    # Training parameters
    ...
    parser.add_argument('--fine_tune_encoder_all', type=bool, default=False,
                        help='whether fine-tune all the parameters')
    parser.add_argument('--fine_tune_embedding', type=bool, default=False,
                        help='whether fine-tune word embeddings or not')
    parser.add_argument('--cnn_name', default='resnet50')
    ...
    if args.fine_tune_encoder:
        file_name = file_name + '_fine_tune_fed'
    if args.fine_tune_encoder_all:
        file_name = file_name + '_fine_tune_all_fed'
    if args.mode == "drpe":
        encoder = CNN_Encoder2()
    else:
        encoder = CNN_Encoder()
    if args.fine_tune_encoder:
    ...
    train_dataset = CaptionFedBaseDataset(args.data_folder, args.file_name, 'TRAIN', args.mode)

    val_loader = torch.utils.data.DataLoader(
        CaptionDataset(args.data_folder, args.file_name, 'VAL', args.mode),
    ...
    with h5py.File(os.path.join('models/' + args.data_name + '/' + str(args.image_size) + '/' +
                                'models_' + args.mode + '/' + str(args.num_clients) + '/'
                                + 'metrics_fed.hdf5'), 'a') as h_:
