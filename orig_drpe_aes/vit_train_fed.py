...
from utils_v import AverageMeter, clip_gradient, accuracy, get_eval_score, save_fl_checkpoint
            ...
            for epoch_l in range(args_.epoch_local):
                ...
                if args_.mode == "drpe":
                    for i_t, (real_image, imag_image, caps, caplens) in enumerate(client_dataloader):
                        ...
                        feat = local_encoder.forward_features(torch.cat((real_image, imag_image), dim=1))
                    ...
                    for i_t, (image, caps, caplens) in enumerate(client_dataloader):
                        ...
                        feat = local_encoder.forward_features(image)
                        scores, caps_sorted, decode_lengths, alphas, sort_ind = local_decoder(feat, caps, caplens)
                        targets = caps_sorted[:, 1:]
                        scores = pack_padded_sequence(scores, decode_lengths, batch_first=True).data
                        targets = pack_padded_sequence(targets, decode_lengths, batch_first=True).data
    ...
    with torch.no_grad():
        ...
        if args_.mode == "drpe":
            for i_t, (real_image, imag_image, caps, caplens, allcaps) in enumerate(val_loader_):
                ...
                feat = encoder_.forward_features(torch.cat((real_image, imag_image), dim=1))
...
if __name__ == '__main__':
    ...
    if args.mode == "drpe":
        encoder = timm.create_model("vit_base_patch16_224", pretrained=True, in_chans=6)
    else:
