...
class DecoderLayer(nn.Module):
    def __init__(self, embed_dim, dropout, n_heads):
        ...
        self.dec_enc_attn = Multi_Head_Attention(Q_dim=embed_dim, K_dim=2048, QKVdim=64, n_heads=n_heads,
                                                 dropout=dropout)
        self.pos_ffn = PoswiseFeedForwardNet(embed_dim=embed_dim, d_ff=2048 * 2, dropout=dropout)
...
class Encoder(nn.Module):
    ...
    def get_position_embedding_table(self):
        ...
        def get_posi_angle_vec(position):
            return [cal_angle(position, hid_idx)[0] for hid_idx in range(1024)] + [cal_angle(position, hid_idx)[1]
