...
def save_checkpoint(file_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
                    decoder_optimizer, encoder_scheduler, decoder_scheduler,
                    metrics, is_best_bleu, is_best_cider, mode, image_size, data_name):
    state = {'epoch': epoch,
    ...
    folder = 'models/' + data_name + '/' + str(image_size) + '/' + 'models_' + mode
...
def save_fl_checkpoint(file_name, epoch, epochs_since_improvement, encoder, decoder, metrics, is_best_bleu,
                       is_best_cider, mode, image_size, data_name, clients):
    state = {'epoch': epoch,
             'epochs_since_improvement': epochs_since_improvement,
             'metrics': metrics,
             'encoder': encoder,
             ...
             'data_name': data_name}
    filename = 'checkpoint_' + file_name + '.pth.tar'
    folder = 'models/' + data_name + '/' + str(image_size) + '/' + 'models_' + mode + '/' + str(
        clients)
    if not os.path.exists(folder):
        os.makedirs(folder)
    torch.save(state, folder + '/' + filename)
