...
if __name__ == '__main__':
    ...
    if args.fine_tune_encoder_all:
        file_name = file_name + '_coco_fine_tune_all'
    file_name = file_name + '_' + str(args.encoder_layers) + '_' + str(args.decoder_layers)
    checkpoint = torch.load(args.checkpoint, map_location=str(device))
    start_epoch = 0
    encoder = checkpoint['encoder']
    decoder = checkpoint['decoder']
    decoder.decoder.vocab_size = len(word_map)
    decoder.decoder.tgt_emb = nn.Embedding(len(word_map), args.emb_dim, padding_idx=0)
    decoder.decoder.projection = nn.Linear(args.emb_dim, len(word_map), bias=False)
    if args.fine_tune_encoder_all:
    ...
    for epoch in range(start_epoch, args.epochs):
        train(args, train_loader_=train_loader, encoder_=encoder, decoder_=decoder, criterion_=criterion,
              encoder_optimizer_=encoder_optimizer, decoder_optimizer_=decoder_optimizer, epoch_=epoch)
        metrics = validate(args, val_loader_=val_loader, encoder_=encoder, decoder_=decoder, criterion_=criterion)
        ...
        save_checkpoint(file_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
                        ...
                        args.photon, args.image_size, args.data_name, args.n_heads)
