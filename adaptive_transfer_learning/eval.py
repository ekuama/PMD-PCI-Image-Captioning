...
def evaluate(args_, photon):
    beam_size = args_.beam_size
    Caption_End = False
    loader = torch.utils.data.DataLoader(
        CaptionDataset(args_.data_folder, args_.file_name, args_.data_name, 'TEST', photon, args_.image_size),
...
if __name__ == '__main__':
    ...
    for pn in photon_numbers:
        checkpoint = (args.base_path + '/' + args.data_name + '/' + args.image_size + '/models_'
                      + "{:.2f}".format(pn) + "/" + args.checkpoint_name)
        print(checkpoint)
