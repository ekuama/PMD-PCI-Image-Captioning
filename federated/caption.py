...
def evaluate(args_, word_map_, pn_, encoder_1, decoder):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cnn_name', default="resnet", help='which model does cnn use?')
    ...
    for photon_number in photon_numbers:
        ...
        checkpoint_path = (args.base_path + '/' + + args.image_size + '/models_' + "{:.2f}".format(photon_number) +
                           "/" + str(args.clients) + args.checkpoint_name)
        ...
        encoder_.eval()
        image_names, sequences = evaluate(args, word_map, photon_number, encoder_, decoder_)
        ...
        folder = ('results_csv/' + args.data_name + '/' + str(args.image_size) + '/' + str(args.clients) + '/'
                  + 'models_' + "{:.2f}".format(photon_number))
